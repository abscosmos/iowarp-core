...
class MemBdevTransport : public BdevTransport {
 ...
 private:
  ...
  // A RAM page. A kPinned pool allocates page-locked host memory through GpuApi
  // (cudaMallocHost / hipHostMalloc / sycl::malloc_host) so GPU DMA runs truly
  // asynchronously and concurrent transfers overlap; every other pool — and any
  // build or host without a usable GPU backend — uses ordinary pageable
  // `new char[]`. The `pinned` flag records which allocator owns `data` so it is
  // released through the matching free path.
  //
