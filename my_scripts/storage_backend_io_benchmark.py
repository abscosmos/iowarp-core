...
class StorageBackendBenchmark(ABC):
    ...
    def _submit_put_tasks(
        ...
        with ThreadPoolExecutor(max_workers=self.concurrency) as ex:
            futures = [
                ex.submit(submit_slice, start, end) for start, end in slices
            ]
    ...
    def run(self) -> dict:
        ...
        needs_full_cpu_buffer = rust_raw or "clio_cte" in self.backend_name

        required_buffer_gb = (
            1.1
            * _compute_required_buffer_gb(metadata, self.num_ops, self.chunk_size)
            if needs_full_cpu_buffer
            else 0.1
        )
    ...
    def _execute_read_phase(self) -> list[tuple[CacheEngineKey, Optional[MemoryObj]]]:
        ...
        with ThreadPoolExecutor(max_workers=self.concurrency) as ex:
            futures = [
                ex.submit(submit_read_slice, start, end) for start, end in slices
            ]
            for future in futures:
                future.result()
...
# ============================================================================
# ClioCTEBackend Implementation
# ============================================================================
class ClioCTEBackendBenchmark(StorageBackendBenchmark):
    """Benchmark for the CLIO CTE storage backend."""
    ...
    def __init__(
        ...
        self.pool_query_mode = pool_query_mode
        self.max_inflight = max_inflight

    @property
    def extra_config_keys(self) -> dict:
        """Return configuration consumed by ClioCTEBackend."""
        return {
            "clio_cte.tag_name": self.tag_name,
            "clio_cte.pool_query_mode": self.pool_query_mode,
            "clio_cte.max_inflight": self.max_inflight,
        }
    ...
    def _create_backend(
        ...
        """Create the CLIO CTE backend after selecting it on the CLI."""
        from lmcache.v1.storage_backend.plugins.clio_cte_backend import (
            ClioCTEBackend,
        )
    ...
    def _wait_for_completion(self, pending_ops: list[Any]) -> None:
        """Fail immediately when a synchronous CTE batch has partial writes."""
        del pending_ops
        if self._completed != self.num_ops:
            raise RuntimeError(
                f"{self.backend_name} write failed: "
                f"completed={self._completed}, expected={self.num_ops}"
            )

    def _close_backend(self) -> None:
        """Close the CLIO CTE store."""
        self._backend.close()

    def _get_extra_result_fields(self) -> dict:
        """Report CLIO CTE concurrency configuration."""
        return {
            "clio_cte_tag_name": self.tag_name,
            "clio_cte_pool_query_mode": self.pool_query_mode,
            "clio_cte_max_inflight_per_thread": self.max_inflight,
            "clio_cte_max_effective_inflight": self.concurrency
            * self.max_inflight,
        }
...
def main() -> None:
    ...
    parser.add_argument(
        "--clio-cte-tag-name",
        type=str,
        default=f"lmcache_storage_bench_{os.getpid()}",
        help="CTE tag used for benchmark records",
    )
    parser.add_argument(
        "--clio-cte-pool-query-mode",
        choices=["local", "dynamic", "broadcast"],
        default="local",
        help="CTE routing mode",
    )
    parser.add_argument(
        "--clio-cte-max-inflight",
        type=int,
        default=16,
        help="Maximum CTE operations in flight per submit thread",
    )
    parser.add_argument(
        "--remote-url",
        type=str,
    ...
    if args.num_ops < 1:
        parser.error("--num-ops must be at least 1")
    ...
    results = []
    if args.backend == "clio_cte":
        if args.clio_cte_max_inflight < 1:
            parser.error("--clio-cte-max-inflight must be at least 1")
        clio_cte_bench = ClioCTEBackendBenchmark(
            num_ops=args.num_ops,
            concurrency=args.concurrency,
            alignment=args.alignment,
            tag_name=args.clio_cte_tag_name,
            pool_query_mode=args.clio_cte_pool_query_mode,
            max_inflight=args.clio_cte_max_inflight,
            write_bench=write_bench,
            chunk_size=args.chunk_size,
            verify_integrity=args.verify_integrity,
        )
        result = clio_cte_bench.run()
        result.update(clio_cte_bench._get_extra_result_fields())
