...
# Select recipe directory based on preset
# CUDA builds use a separate recipe with relaxed CMAKE_FIND_ROOT_PATH
# so the system CUDA toolkit (outside conda prefix) can be found
if [ "$PRESET" = "cuda" ] || [[ "$PRESET" == cuda-* ]]; then
    RECIPE_DIR="$SCRIPT_DIR/installers/conda-cuda"
    # Map shorthand "cuda" to the actual preset name
    if [ "$PRESET" = "cuda" ]; then
        PRESET="cuda-delta"
    fi
else
...
echo -e "${BLUE}Using recipe: $RECIPE_DIR${NC}"
echo -e "${BLUE}Using preset: $PRESET${NC}"
echo ""
