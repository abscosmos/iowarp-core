...
class CfsIo {
 public:
  ...
  /** rename(2): both paths must carry the clio:: marker. */
  int RenamePath(const std::string &raw_src, const std::string &raw_dst);
  /** readdir(3) over one directory: direct children, names only. */
  int Readdir(const std::string &raw_path, std::vector<std::string> *out);
  ...
  int StatPath(const std::string &raw_path, StatT *buf) {
    ...
    clio::run::u64 size = 0;
    bool exists = false;
    if (!CLIO_CFS_CLIENT->GetAttr(path, &exists, &size) || !exists) {
 ...
 private:
  // The adapter's ENTIRE state: the descriptor table. Everything an earlier
  // version kept here — the per-path in-flight write window, the staging
  // allocator, the drain/reap helpers, the size queries and the zero-IPC read
  // path — now lives in clio::cte::filesystem::Client, where every adapter
  ...
  /** Copy out the descriptor's state, or set EBADF and return false. */
  bool Lookup(int fd, OpenFile *out);

  /** Advance a descriptor's seek offset after a successful read/write. */
  void Advance(int fd, clio::run::u64 n);
